...
public class LabelAdapter extends ArrayAdapter<Label> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			viewHolder.modelName = (TextView) view
					.findViewById(R.id.model_name);
			viewHolder.mycode = (TextView) view.findViewById(R.id.mycode);
			viewHolder.patternName = (TextView) view
					.findViewById(R.id.pattern_name);
			viewHolder.workStatus = (TextView) view
		...
		if (label.getGood() != null)
			viewHolder.goodsName.setText(STR_GOODS__NAME
					+ label.getGood().getBarCode() + "__"
					+ label.getGood().getEslName() + "__"
					+ label.getGood().getPresPrice());
		else
			viewHolder.goodsName.setText(STR_GOODS__NAME);
		if (label.getModel() != null)
			viewHolder.modelName.setText(STR_MODEL_NAME
					+ label.getModel().getModelName());
		else
			viewHolder.modelName.setText(STR_MODEL_NAME);
		viewHolder.mycode.setText(STR_MYCODE + label.getMycode());
		if (label.getPattern() != null)
			viewHolder.patternName.setText(STR_PATTERN__NAME
					+ label.getPattern().getPatternName());
		else
			viewHolder.patternName.setText(STR_PATTERN__NAME);
		viewHolder.workStatus.setText(STR_WORK__STATUS
				+ (label.getWorkStatus() == 0 ? "工作中" : "停用"));
